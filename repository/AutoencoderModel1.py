...
class PReLU1(nn.Module):
    ...
    def __init__(self, input_shape, init = 0.25,
                 device=None, dtype=None):
        factory_kwargs = {'device': device, 'dtype': dtype}
        self.input_shape = input_shape
        super(PReLU1, self).__init__()
        self.weight = nn.parameter.Parameter(torch.empty(input_shape, **factory_kwargs).fill_(init))

    def forward(self, x):
        return torch.where(x<0,torch.mul(x,self.weight),x)

    def extra_repr(self):
        return 'input_shape={}'.format(self.input_shape)
...
class Decoder(nn.Module):
    def __init__(self,c):
        ...
        self.layers = nn.Sequential(OrderedDict([
            ...
            ('prelu1', PReLU1(input_shape=(256,8,8))),
            ...
            ('prelu4', PReLU1(input_shape=(256,16,16))),
...
class Autoencoder1(nn.Module):
    ...
    def calculate_filter_num(self, bw_ratio, input_cwh):
        ...
        filter_num = torch.prod(torch.tensor(input_cwh))*bw_ratio*2/torch.prod(dimensions5)
...
def train_one_epoch(model, loader, loss_func, optimizer, epochinfo):
    ...
    for batch_idx, (x, _) in enumerate(loader):
        ...
        if batch_idx%100==0:
            msg = (f"[{dev}] [SNR {snrdb_train}] [****Train] "+
...
def test_one_epoch(model, loader, loss_func, epochinfo):
    ...
    msg = (f"[{dev}] [SNR {snrdb_train}] [----Test]  "+ 
           f"[Epoch {epochinfo[0]+1:2}|{epochinfo[1]}] "+
           f"[Loss {loss:.5f}] "+
           f"[PSNR {psnr:.2f}]\n\n")
    print(msg)
...
if __name__ == "__main__":
    ...
    if load_pretrained:
        with open(ws_root+
                  'ae_torch/savedmodels1_test/snrdb_inf_bwratio_0.167_2024_01_15_20_31_23.pth.tar'
                  ,'rb') as f:
        ...
        autoencoder = Autoencoder1(snrdb_train,bw_ratio,(3,32,32))
    ...
    for epoch in tqdm.trange(epoch_num):
        ...
        if loss_test<bestloss:
            ...
            modelfilename = f'ae_torch/savedmodels1_test/snrdb_{snrdb_train}_bwratio_{bw_ratio:.3f}_{timepref()}.pth.tar'
            with open(ws_root+modelfilename,'wb') as f:
                pickle.dump(autoencoder,f)
        if 'logfilename' in dir():
            os.remove(ws_root+logfilename)
        logfilename = f'ae_torch/savedmodels1_test/psnr_snrdb_{snrdb_train}_bwratio_{bw_ratio:.3f}_{timepref()}.txt'
